...
const AskCropSentinel = () => {
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
      const history = messages.map(m => ({ role: m.sender === 'ai' ? 'model' : 'user', parts: [m.text] }));
      const response = await api.askCropSentinel(textToSend.trim(), history);
      ...
      <main className="flex-1 p-16 md:p-32 lg:p-48 max-w-[1400px] h-[calc(100vh-60px)] lg:h-screen flex flex-col">
        ...
        <div className="flex-1 bg-white rounded-[12px] border border-stone/10 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="px-32 py-24 border-b border-stone/10 bg-[#F9F9F9] flex items-center gap-16 shrink-0">
            <div className="w-[40px] h-[40px] bg-turmeric-gold rounded-full flex items-center justify-center font-display font-bold text-soil-ink text-[20px]" aria-hidden="true">
              F
            </div>
            <div>
          ...
          {/* Chat History Area */}
          <div className="flex-1 overflow-y-auto p-32 flex flex-col gap-24 bg-white" ref={scrollRef} aria-live="polite">
            {messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center max-w-[600px] mx-auto">
                <span className="material-symbols-outlined text-[48px] text-stone/30 mb-16" aria-hidden="true">chat_bubble</span>
              ...
              <>
                ...
                {isProcessing && (
                  <div className="flex w-full justify-start" aria-label="AI is typing" role="status">
          ...
          <div className="p-24 border-t border-stone/10 bg-white shrink-0">
            <div className="relative max-w-[900px] mx-auto">
              <input 
                ...
                aria-label="Message CropSentinel AI"
              />
              <button 
                onClick={() => handleSend()}
