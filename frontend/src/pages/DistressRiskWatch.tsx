...
import type { DashboardMetrics, AIBrief, TopMover } from '../services/api';
...
const DistressRiskWatch = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [metricsData, briefData, moversData] = await Promise.all([
       ...
       <main className="flex-1 p-16 md:p-32 lg:p-48 max-w-[1400px]">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-24 lg:mb-48 gap-16 lg:gap-0">
            ...
            <div className="flex items-center gap-8 bg-white border border-stone/20 rounded-full px-16 py-12 shadow-sm">
              <span className="material-symbols-outlined text-[20px] text-stone" aria-hidden="true">calendar_month</span>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-64" role="status" aria-label="Loading dashboard data">
            ...
            <div className="bg-chili-vermillion/10 border border-chili-vermillion/20 rounded-[12px] p-24 flex items-center gap-16" role="alert">
            ...
            <>
              {/* 6 Key Metrics Row */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-16 mb-24 lg:mb-48" aria-label="Key metrics">
                ...
                <div className="bg-white rounded-[12px] border border-stone/10 p-24 shadow-sm flex flex-col">
                  ...
                  <div className="flex items-center gap-4 text-[13px] font-bold text-board-green">
                    <span className="material-symbols-outlined text-[16px]" aria-hidden="true">trending_up</span> {metrics?.total_farmers.subtitle}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-[12px] border border-stone/10 p-24 shadow-sm flex flex-col">
                  ...
                  <div className="flex items-center gap-4 text-[13px] font-bold text-board-green">
                    <span className="material-symbols-outlined text-[16px]" aria-hidden="true">trending_up</span> {metrics?.alerts_generated.subtitle}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-[12px] border border-stone/10 p-24 shadow-sm flex flex-col">
                  ...
                  <div className="flex items-center gap-4 text-[12px] font-bold text-chili-vermillion bg-chili-vermillion/10 px-8 py-4 rounded-md w-fit">
                    <span className="material-symbols-outlined text-[14px]" aria-hidden="true">warning</span> {metrics?.distress_markets.subtitle}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-[12px] border border-stone/10 p-24 shadow-sm flex flex-col">
                  ...
                  <div className="flex items-center gap-4 text-[13px] font-bold text-board-green">
                    <span className="material-symbols-outlined text-[16px]" aria-hidden="true">check_circle</span> {metrics?.prediction_accuracy.subtitle}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-32">
                ...
                <div className="lg:col-span-2 bg-white rounded-[12px] border border-stone/10 overflow-hidden shadow-sm flex flex-col">
                  <div className="px-32 py-24 flex justify-between items-center border-b border-stone/10 bg-[#F9F9F9]">
                    <h2 className="font-display font-bold text-[20px] text-soil-ink flex items-center gap-8">
                      Top Market Movements <span className="material-symbols-outlined text-board-green text-[18px]" aria-hidden="true">trending_up</span>
                    </h2>
                  </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-stone/10">
                          <th scope="col" className="px-32 py-16 text-[12px] font-bold text-stone uppercase tracking-widest bg-white">Commodity</th>
                          <th scope="col" className="px-32 py-16 text-[12px] font-bold text-stone uppercase tracking-widest bg-white">Market</th>
                          <th scope="col" className="px-32 py-16 text-[12px] font-bold text-stone uppercase tracking-widest bg-white text-right">Live Price</th>
                        </tr>
                      </thead>
                        ...
                        {topMovers.length > 0 ? topMovers.map((item, idx) => (
                          <tr key={idx} className="hover:bg-stone/5 transition-colors">
                            ...
                            <td className="px-32 py-16 text-right">
                              <span className="font-mono font-bold text-board-green text-[15px]">₹{item.price.toFixed(2)}</span>
                            </td>
                          </tr>
                        )) : (
                          <tr>
                            <td colSpan={3} className="px-32 py-32 text-center text-stone text-[14px]">No live market data available.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                ...
                <div className="bg-white rounded-[12px] border border-stone/10 shadow-sm flex flex-col h-full border-t-[4px] border-t-turmeric-gold">
                  <div className="px-32 py-32 flex-1" aria-live="polite">
                    <h2 className="font-display font-bold text-[18px] text-soil-ink flex items-center gap-12 mb-32">
                      <span className="material-symbols-outlined text-turmeric-gold text-[24px]" aria-hidden="true">auto_awesome</span>
                  ...
                  <div className="px-32 py-24 border-t border-stone/10 flex items-center justify-between bg-[#F9F9F9]">
                    ...
                    <button className="text-[13px] font-bold text-board-green hover:underline font-display" aria-label="View full AI report">View Full Report</button>
                  </div>
                </div>
                )}
